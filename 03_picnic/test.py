...
# --------------------------------------------------
def test_non_oxford():
    """test turning off the oxford comma"""
    arg = 'bananas apples dates cherries'
    out = getoutput(f'{prg} {arg} --oxoff --sorted')
    expected = ('You are bringing apples, bananas, cherries and dates.')
    assert out.strip() == expected

# --------------------------------------------------
def test_alt_separator():
    """test passing an alternative separator character"""
    arg = 'bananas apples dates cherries'
    separators = [";", "/", ",", "."]
    for sep in separators:
        out = getoutput(f'{prg} {arg} --sep \"{sep}\" --sorted')
        expected = (f'You are bringing apples{sep} bananas{sep} cherries{sep} and dates.')
        assert out.strip() == expected
